...
if (strcmp ($res, "VERIFIED") == 0) {
    ...
    //else {
        ...
        $sql = $dbh->prepare("SELECT * FROM transactions WHERE txn_id = ?");
       
        if ($sql->execute(array($txn_id))) {
           $count=$sql->rowCount();
        }
        ...
        if($count > 0) {
            ...
            $statement = $dbh->prepare("SELECT * FROM orders WHERE orderid = ?");
            ...
            for($i = 1; $i <= $num_cart_items; $i++)
            {
                ...
                $product_list .= $item_number.'_'.$quantity.'_'.$price.';';
                $message .= "," .$item_number.",".$quantity.",".$price;
            }
            $message .= ",".number_format($payment_amount);
...
echo "<script type='text/javascript'>console.log('The IPN is invalid');</script>";
        ...
        $statement = $dbh->prepare("INSERT INTO transactions(txn_id, product_list) VALUES (?, ?)");
        
        if($statement->execute(array($txn_id, $product_list))){
